defmodule Tzn.PodGroupReporting do
  ...
  def student_highlights(%Pod{} = pod) do
    ...
    missed_sessions_this_month = pod.timesheet_entries |> Enum.filter(fn tse -> tse.category == "missed_session" && Tzn.Util.within_n_days_ago(tse.started_at, 30) end) |> Enum.count()
    missed_meetings_in_a_row = pod.timesheet_entries |> Enum.sort_by(& &1.started_at, {:desc, NaiveDateTime}) |> Enum.take_while(& &1.category == "missed_session") |> Enum.count()


    hours_mentored_hit_5_at  = pod.timesheet_entries
      |> Enum.sort_by(& &1.started_at, {:asc, NaiveDateTime})
      |> Enum.reduce_while({0.0, nil}, fn tse, {hours_mentored, _} = acc ->
        if hours_mentored > 5.0 do
          {:halt, acc}
        else
          {:cont, {hours_mentored + (Timex.diff(tse.ended_at, tse.started_at, :duration) |> Timex.Duration.to_hours()), tse.started_at}}
        end
      end)
      |> then(fn {hours, date} ->
        if hours > 5.0 do
    ...
    hours_mentored_hit_10_at  = pod.timesheet_entries
      |> Enum.sort_by(& &1.started_at, {:asc, NaiveDateTime})
      |> Enum.reduce_while({0.0, nil}, fn tse, {hours_mentored, _} = acc ->
        if hours_mentored > 5.0 do
          {:halt, acc}
