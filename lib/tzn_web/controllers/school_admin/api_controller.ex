defmodule TznWeb.SchoolAdmin.ApiController do
  ...
  def handle(conn, %{"query" => "cohorts_and_students"}) do
    json(
      conn,
      Enum.map(conn.assigns.data, fn pod_group ->
        %{
          id: pod_group.id,
          name: pod_group.name,
          students:
            Enum.map(pod_group.pods, fn pod ->
              %{
                id: pod.id,
                ...
                first_meeting_with_mentor:
                  ...
                  |> List.first(%{})
                  |> Map.get(:started_at),
                most_recent_meeting_with_mentor:
                  pod.timesheet_entries
                  |> Enum.sort_by(& &1.id)
                  |> List.last(%{})
                  |> Map.get(:started_at),
          ...
          school_admins:
            Enum.map(pod_group.school_admins, fn sa ->
              %{
                name: sa.name
              }
            end)
  ...
  def handle(conn, %{"query" => "num_students"}) do
    data = %{
  ...
  def handle(conn, %{"query" => "hours_mentored"}) do
    data = %{
